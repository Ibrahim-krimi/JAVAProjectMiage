...
package miage.TDAnnotations.exercice1;
...
public class AnalyseurDeClasseIncludingAnnotation {
	...
	/**
	 * Cette m̩thode affiche par ex "public class Toto extends Tata implements
	 * Titi, Tutu {"
	 */
	public static void afficheEnTeteClasse(Class<?> cl) {

		Annotation[] classAnnotations = cl.getDeclaredAnnotations();
	...
	public static void afficheConstructeurs(Class<?> cl) {
	  ...
	  for(Constructor<?> c : constructors) {
		  ...
		  afficheParams(c, c.getParameterTypes());
	  }
  }

	private static void afficheParams(Executable cl, Class<?>[] params) {


		System.out.print("(");
		String sep = "";
		for (Class<?> c : params) {
			// Ajout pour inclure les annotations (ajoute en faite les annotations de
			// classe.... faux)
...
//		  }
		...
		System.out.println(");");
		Annotation[][] parameterAnnotations = cl.getParameterAnnotations();
		if (parameterAnnotations.length > 0) {
			//System.out.println();
			for (int i = 0; i < parameterAnnotations.length; i++) {
				...
				if (oneParameterAnnotations.length > 0) {
					System.out.println();
					for (int j = 0; j < oneParameterAnnotations.length; j++) {
						System.out.println(oneParameterAnnotations[j].toString());
					}
				}
				else System.out.println("pas d'annotation");
			}
		}
	}
	...
	public static void afficheMethodes(Class<?> cl) {
		...
		for (Method m : methods) {
			...
			afficheParams(m, m.getParameterTypes());
		}
	}
